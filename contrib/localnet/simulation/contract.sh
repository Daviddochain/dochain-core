...
for j in $(seq 0 1); do
	...
	id=$(dochaind q tx $txhash -o json --node $(sh $SIMULATION_FOLDER/next_node.sh) | jq -r '.raw_log' | jq -r '.[0].events[1].attributes[1].value')
	...
	sleep 10
	txhash=$(echo $out | jq -r '.txhash')
	contract_addr=$(dochaind q tx $txhash -o json --node $(sh $SIMULATION_FOLDER/next_node.sh) | jq -r '.raw_log' | jq -r '.[0].events[0].attributes[3].value')
	...
	echo "... send tokens"
	for i in $(seq 0 3); do
		peer_addr=$(dochaind keys show test$i -a --home $NODE_HOME --keyring-backend $KEYRING_BACKEND)

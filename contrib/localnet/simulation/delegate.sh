...
do
    for i in $(seq 1 3); do
        ...
        balance=$(dochaind q bank balances $(dochaind keys show test$i -a --keyring-backend $KEYRING_BACKEND --home $NODE_HOME) --node $(sh $SIMULATION_FOLDER/next_node.sh) -o json | jq -r '.balances | if length > 0 then .[] | select(.denom == "udo").amount else "0" end')

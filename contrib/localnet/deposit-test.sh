...
dochaind="docker exec dochainnode1 ./dochaind"
...
#
# 100 block from now
STATUS_INFO=($($dochaind status --home $NODE1_HOME | jq -r '.NodeInfo.network,.SyncInfo.latest_block_height'))
CHAIN_ID=${STATUS_INFO[0]}

#
# initial prop should pass and increases min initial deposit ratio to 0.1
STATUS=$($dochaind tx gov submit-proposal param-change $NODE1_HOME/prop.json --keyring-backend test --yes --from node1 --broadcast-mode block --chain-id $CHAIN_ID --home $NODE1_HOME -o json | jq -r .code)
if [ $STATUS -ne 0 ]; then
...
for i in {0..5}; do
    ...
    if [[ $(docker ps -a | grep dochainnode$i | wc -l) -eq 1 ]]; then
		sleep 5
        $dochaind tx gov vote 1 yes --from node$i --keyring-backend test --chain-id $CHAIN_ID --home "node$i/dochaind" -y
    fi
done
...
for i in {0..6}; do
    STATUS=$($dochaind q gov proposal 1 --chain-id $CHAIN_ID --home $NODE1_HOME -o json | jq -r .status)
    echo $STATUS
...
STATUS=$($dochaind tx gov submit-proposal param-change $NODE1_HOME/prop2.json --keyring-backend test --yes --from node1 --broadcast-mode block --chain-id $CHAIN_ID --home $NODE1_HOME -o json | jq -r .code)
if [ $STATUS -ne 0 ]; then
...
#
# retry - this time with 100000 udotest - insufficient deposit
STATUS=$($dochaind tx gov submit-proposal param-change $NODE1_HOME/prop3.json --keyring-backend test --yes --from node1 --broadcast-mode block --chain-id $CHAIN_ID --home $NODE1_HOME -o json | jq -r .code)
if [ $STATUS -ne 0 ]; then
...
#
# retry - this time with 1000000 udotest - sufficient deposit
STATUS=$($dochaind tx gov submit-proposal param-change $NODE1_HOME/prop4.json --keyring-backend test --yes --from node1 --broadcast-mode block --chain-id $CHAIN_ID --home $NODE1_HOME -o json | jq -r .code)
if [ $STATUS -ne 0 ]; then

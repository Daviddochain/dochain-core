...
update_test_genesis () {
    ...
    cat ~/.dochain/config/genesis.json | jq --arg DENOM "$2" "$1" > ~/.dochain/config/tmp_genesis.json && mv ~/.dochain/config/tmp_genesis.json ~/.dochain/config/genesis.json
}
...
for i in $(seq 0 3); do
    ...
    # Add new account
    echo $mnemonic | dochaind keys add $keyname --keyring-backend $KEYRING --recover --home ~/.dochain
    # Add initial balances
    dochaind add-genesis-account $keyname "1000000000000uluna" --keyring-backend $KEYRING --home ~/.dochain
done

# Sign genesis transaction
dochaind gentx test "1000000uluna" --keyring-backend $KEYRING --chain-id $CHAINID --home ~/.dochain
...
# Collect genesis tx
dochaind collect-gentxs --home ~/.dochain

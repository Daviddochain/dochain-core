...
if [ ! -f "$BUILDDIR/old/dochaind" ]; then
    ...
    docker build --platform linux/amd64 --no-cache --build-arg source=./_build/core-${OLD_VERSION:1}/ --tag classic-do/terraclassic.dochaind-binary.old -f contrib/updates/Dockerfile.old .
    docker create --platform linux/amd64 --name old-temp classic-do/terraclassic.dochaind-binary.old:latest
...
if [ ! -f "$BUILDDIR/node0/dochaind/config/genesis.json" ]; then docker run --rm \
    ...
    classic-do/dochaind-upgrade-env testnet --v $TESTNET_NVAL --chain-id $TESTNET_CHAINID -o . --starting-ip-address 192.168.10.2 --keyring-backend=test --home=temp; \
fi
...
for (( i=0; i<$TESTNET_NVAL; i++ )); do
    ...
    docker run --rm \
        ...
        --platform linux/amd64 \
        classic-do/dochaind-upgrade-env init /dochaind/old/dochaind

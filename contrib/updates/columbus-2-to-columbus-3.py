...
def process_raw_genesis(genesis, parsed_args):
    ...
    for acc in genesis['app_state']['accounts']:
        ...
        if acc['address'] == 'do1dp0taj85ruc299rkdvzp4z5pfg6z6swaed74e6':
            # (Foundation) sub 978,260 dotest to give new vesting account
            for coin in newAcc['coins']:
              if coin['denom'] == 'udotest':
        ...
        if acc['address'] == 'do10y5usrnwk2ltddm5kenhznl5uj6w3yfga5al4a':
            # 978,260 dotest 1M, 2M, 3M, 12M 10% 10% 10% 70%
            coin = {
              'denom': 'udotest',
              'amount': '978260000000'
            }
    ...
    newAccounts.append(
        create_module_account(
            ...
            [{'amount': str(bondedAmt), 'denom': 'udotest'}], 
            ['burner', 'staking']
        )
    )
    ...
    newAccounts.append(
        create_module_account(
            ...
            [{'amount': str(notBondedAmt), 'denom': 'udotest'}], 
            ['burner', 'staking']
        )
    )
    ...
    genesis['app_state']['oracle'] = {
        ...
        'params': {
            ...
            'whitelist': ['ukrw', 'usdr', 'udotest', 'umnt'],
            'reward_band': '0.07',                     # 7%
...
def update_vesting_schedule(account):
    ...
    do_vesting_schedule = {
        'denom': 'usdr',
        'schedules': do_schedules
    }

    vesting_schedules.append(luna_vesting_schedule)
    vesting_schedules.append(do_vesting_schedule)
...
def update_seed_to_private_vesting_schedule(account):
    ...
    for vesting_schedule in account['vesting_schedules']:
        if vesting_schedule['denom'] == 'usdr':
            do_vesting_schedule['schedules'] = vesting_schedule['schedules']
            break

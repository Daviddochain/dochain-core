...
func (s *WasmTestSuite) TestTax() {
	...
	// fund an account
	s.FundAcc(payer, sdk.NewCoins(sdk.NewInt64Coin(core.MicroDoDenom, 1000000000)))

	// instantiate reflect contract
	contractAddr := s.InstantiateContract(payer, DoBindingsPath)
	s.Require().NotEmpty(contractAddr)

	// make a bank send message
	coin := sdk.NewInt64Coin(core.MicroDoDenom, 10000)
	taxAmount := sdkmath.LegacyNewDecFromInt(coin.Amount).Mul(taxRate).TruncateInt()
	updateAmt := coin.Amount.Add(taxAmount)
	updatedCoins := sdk.NewCoins(sdk.NewInt64Coin(core.MicroDoDenom, updateAmt.Int64()))
	...
	s.Require().Equal(sdkmath.NewInt(1000000000).Sub(updateAmt), res.AmountOf(core.MicroDoDenom))
}

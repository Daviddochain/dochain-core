...
## Swap Fees
Since dochain's price feed is derived from validator oracles, there is necessarily a delay between the on-chain reported price and the actual realtime price.
...
* a Tobin Tax (set at 0.25%) for spot-converting dochain<>dochain swaps
...
* a minimum spread (set at 2%) for dochain<>Luna swaps
...
dochain uses a Constant Product market-making algorithm to ensure liquidity for dochain<>Luna swaps.

With Constant Product, we define a value `CP` set to the size of the dochain pool multiplied by a set fiat value of Luna, and ensure our market-maker maintains it as invariant during any swaps through adjusting the spread.
...
For example, we'll start with equal pools of dochain and Luna, both worth 1000 SDR total. The size of the dochain pool is 1000 SDT, and assuming the price of Luna<>SDR is 0.5, the size of the Luna pool is 2000 Luna. A swap of 100 SDT for Luna would return around 90.91 SDR worth of Luna (≈ 181.82 Luna). The offer of 100 SDT is added to the dochain pool, and the 90.91 SDT worth of Luna are taken out of the Luna pool.
...
The market starts out with two liquidity pools of equal sizes, one representing dochain (all denominations) and another representing Luna, initialiazed by the parameter `BasePool`, which defines the initial size of the dochain and Luna liquidity pools.

In practice, rather than keeping track of the sizes of the two pools, the information is encoded in a number `delta`, which the blockchain stores as `TerraPoolDelta`, representing the deviation of the dochain pool from its base size in units µSDR.

The size of the dochain and Luna liquidity pools can be generated from  using the following formulas:
...
At the end of each block, the market module will attempt to "replenish" the pools by decreasing the magnitude of  between the dochain and Luna pools. The rate at which the pools will be replenished toward equilibrium is set by the parameter `PoolRecoveryPeriod`, with lower periods meaning lower sensitivity to trades, meaning previous trades are more quickly forgotten and the market is able to offer more liquidity.
...
Upon successful completion of dochain<>Luna swaps, a portion of the coins to be credited to the user's account is withheld as the spread fee.

## Seigniorage
For Luna swaps into dochain, the Luna that recaptured by the protocol is burned and is called seigniorage -- the value generated from issuing new dochain. At the end of the epoch, the total seigniorage for the epoch will be calculated and reintroduced into the economy as ballot rewards for the exchange rate oracle and to the community pool by the Treasury module, described more fully [here](../../treasury/spec/README.md).

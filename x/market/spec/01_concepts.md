...
* a minimum spread (set at 2%) for do<>Luna swaps
...
do uses a Constant Product market-making algorithm to ensure liquidity for do<>Luna swaps.

With Constant Product, we define a value `CP` set to the size of the do pool multiplied by a set fiat value of Luna, and ensure our market-maker maintains it as invariant during any swaps through adjusting the spread.
...
For example, we'll start with equal pools of do and Luna, both worth 1000 SDR total. The size of the do pool is 1000 SDT, and assuming the price of Luna<>SDR is 0.5, the size of the Luna pool is 2000 Luna. A swap of 100 SDT for Luna would return around 90.91 SDR worth of Luna (≈ 181.82 Luna). The offer of 100 SDT is added to the do pool, and the 90.91 SDT worth of Luna are taken out of the Luna pool.
...
The market starts out with two liquidity pools of equal sizes, one representing do (all denominations) and another representing Luna, initialiazed by the parameter `BasePool`, which defines the initial size of the do and Luna liquidity pools.

In practice, rather than keeping track of the sizes of the two pools, the information is encoded in a number `delta`, which the blockchain stores as `TerraPoolDelta`, representing the deviation of the do pool from its base size in units µSDR.

The size of the do and Luna liquidity pools can be generated from  using the following formulas:

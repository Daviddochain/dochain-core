...
// GetCmdQueryDoPoolDelta implements the query mint pool delta command.
func GetCmdQueryDoPoolDelta() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "do-pool-delta",
		Args:  cobra.NoArgs,
		Short: "Query do pool delta",
		Long: `Query do pool delta, which is usdr amount used for swap operation from the DoPool.
It can be negative if the market wants more do than Do, and vice versa if the market wants more Do.
...
$ dochaind query market do-pool-delta
	`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			res, err := queryClient.DoPoolDelta(context.Background(),
				&types.QueryDoPoolDeltaRequest{},
			)
			if err != nil {
				return err
			}

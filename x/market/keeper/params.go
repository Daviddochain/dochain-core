...
// BasePool is the market liquidity pool made available per PoolRecoveryPeriod.
func (k Keeper) BasePool(ctx sdk.Context) (res math.LegacyDec) {
	k.paramSpace.Get(ctx, types.KeyBasePool, &res)
	return res
}

// MinStabilitySpread is the minimum spread applied to swaps to or from Do.
// It is intended to reduce swing trades that exploit oracle period delays.
func (k Keeper) MinStabilitySpread(ctx sdk.Context) (res math.LegacyDec) {
	k.paramSpace.Get(ctx, types.KeyMinStabilitySpread, &res)
	return res
}

// PoolRecoveryPeriod is the period required to recover the market pools to the configured base pool level.

...
func (k Keeper) OrganizeBallotByDenom(ctx sdk.Context, validatorClaimMap map[string]types.Claim) (votes map[string]types.ExchangeRateBallot) {
	...
	aggregateHandler := func(voterAddr sdk.ValAddress, vote types.AggregateDoRateVote) (stop bool) {
	...
	k.IterateAggregateDoRateVotes(ctx, aggregateHandler)
...
func (k Keeper) ClearBallots(ctx sdk.Context, votePeriod uint64) {
	...
	k.IterateAggregateDoRatePrevotes(ctx, func(voterAddr sdk.ValAddress, aggregatePrevote types.AggregateDoRatePrevote) (stop bool) {
		if ctx.BlockHeight() > int64(aggregatePrevote.SubmitBlock+votePeriod) {
			k.DeleteAggregateDoRatePrevote(ctx, voterAddr)
		}
	...
	k.IterateAggregateDoRateVotes(ctx, func(voterAddr sdk.ValAddress, aggregateVote types.AggregateDoRateVote) (stop bool) {
		k.DeleteAggregateDoRateVote(ctx, voterAddr)
		return false
	})
}
		...
		for _, item := range whitelist {
			...
			if _, ok := k.bankKeeper.GetDenomMetaData(ctx, item.Name); !ok {
				...
				k.bankKeeper.SetDenomMetaData(ctx, banktypes.Metadata{
					Description: "The native stable token of the do Columbus.",
					...
					Base:    base,
					Display: display,
					Name:    fmt.Sprintf("%s do", strings.ToUpper(display)),

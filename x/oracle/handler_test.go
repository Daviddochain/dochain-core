...
func TestOracleFilters(t *testing.T) {
	...
	*/
	...
	// Case 2: Normal MsgAggregateDoRatePrevote submission goes through
	salt := "1"
	...
	_, err := h.AggregateDoRatePrevote(sdk.WrapSDKContext(input.Ctx), prevoteMsg)
	require.NoError(t, err)

	// Case 3: Normal MsgAggregateDoRateVote submission goes through keeper.Addrs
	voteMsg := types.NewMsgAggregateDoRateVote(salt, randomExchangeRate.String()+core.MicroSDRDenom, keeper.Addrs[0], keeper.ValAddrs[0])
	_, err = h.AggregateDoRateVote(sdk.WrapSDKContext(input.Ctx.WithBlockHeight(1)), voteMsg)
	require.NoError(t, err)
	...
	prevoteMsg = types.NewMsgAggregateDoRatePrevote(hash, sdk.AccAddress(nonValidatorAddr), sdk.ValAddress(nonValidatorAddr))
	_, err = h.AggregateDoRatePrevote(sdk.WrapSDKContext(input.Ctx), prevoteMsg)
	require.Error(t, err)
}
...
func TestFeederDelegation(t *testing.T) {
	...
	prevoteMsg = types.NewMsgAggregateDoRatePrevote(hash, keeper.Addrs[1], keeper.ValAddrs[0])
	_, err = h.AggregateDoRatePrevote(sdk.WrapSDKContext(input.Ctx), prevoteMsg)
	require.Error(t, err)
	...
	voteMsg = types.NewMsgAggregateDoRateVote(salt, randomExchangeRate.String()+core.MicroSDRDenom, keeper.Addrs[1], keeper.ValAddrs[0])
	_, err = h.AggregateDoRateVote(sdk.WrapSDKContext(input.Ctx.WithBlockHeight(1)), voteMsg)
	require.Error(t, err)
	...
	prevoteMsg = types.NewMsgAggregateDoRatePrevote(hash, keeper.Addrs[2], keeper.ValAddrs[0])
	_, err = h.AggregateDoRatePrevote(sdk.WrapSDKContext(input.Ctx), prevoteMsg)
	require.Error(t, err)
	...
	voteMsg = types.NewMsgAggregateDoRateVote(salt, randomExchangeRate.String()+core.MicroSDRDenom, keeper.Addrs[2], keeper.ValAddrs[0])
	_, err = h.AggregateDoRateVote(sdk.WrapSDKContext(input.Ctx.WithBlockHeight(1)), voteMsg)
	require.Error(t, err)
...
func TestAggregatePrevoteVote(t *testing.T) {
	...
	aggregateExchangeRatePrevoteMsg := types.NewMsgAggregateDoRatePrevote(hash, keeper.Addrs[0], keeper.ValAddrs[0])
	_, err := h.AggregateDoRatePrevote(sdk.WrapSDKContext(input.Ctx), aggregateExchangeRatePrevoteMsg)
	require.NoError(t, err)

	// Unauthorized feeder
	aggregateExchangeRatePrevoteMsg = types.NewMsgAggregateDoRatePrevote(hash, keeper.Addrs[1], keeper.ValAddrs[0])
	_, err = h.AggregateDoRatePrevote(sdk.WrapSDKContext(input.Ctx), aggregateExchangeRatePrevoteMsg)
	require.Error(t, err)

	// Invalid reveal period
	aggregateExchangeRateVoteMsg := types.NewMsgAggregateDoRateVote(salt, exchangeRatesStr, keeper.Addrs[0], keeper.ValAddrs[0])
	_, err = h.AggregateDoRateVote(sdk.WrapSDKContext(input.Ctx), aggregateExchangeRateVoteMsg)
	require.Error(t, err)
	...
	aggregateExchangeRateVoteMsg = types.NewMsgAggregateDoRateVote(salt, otherExchangeRateStr, keeper.Addrs[0], keeper.ValAddrs[0])
	_, err = h.AggregateDoRateVote(sdk.WrapSDKContext(input.Ctx), aggregateExchangeRateVoteMsg)
	require.Error(t, err)

	// Invalid exchange rate with valid real period
	input.Ctx = input.Ctx.WithBlockHeight(1)
	aggregateExchangeRateVoteMsg = types.NewMsgAggregateDoRateVote(salt, invalidExchangeRateStr, keeper.Addrs[0], keeper.ValAddrs[0])
	_, err = h.AggregateDoRateVote(sdk.WrapSDKContext(input.Ctx), aggregateExchangeRateVoteMsg)
	require.Error(t, err)

	// Unauthorized feeder
	aggregateExchangeRateVoteMsg = types.NewMsgAggregateDoRateVote(salt, invalidExchangeRateStr, keeper.Addrs[1], keeper.ValAddrs[0])
	_, err = h.AggregateDoRateVote(sdk.WrapSDKContext(input.Ctx), aggregateExchangeRateVoteMsg)
	require.Error(t, err)

	// Unintended denom vote
	aggregateExchangeRateVoteMsg = types.NewMsgAggregateDoRateVote(salt, unintendedExchageRateStr, keeper.Addrs[0], keeper.ValAddrs[0])
	_, err = h.AggregateDoRateVote(sdk.WrapSDKContext(input.Ctx), aggregateExchangeRateVoteMsg)
	require.Error(t, err)

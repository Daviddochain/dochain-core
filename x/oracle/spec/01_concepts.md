...
During each `VotePeriod`, the Oracle module obtains consensus on the exchange rate of Do against denominations specified in `Whitelist` by requiring all members of the validator set to submit a vote for Do exchange rate before the end of the interval.
...
* Prevote and Vote
    ...
    * A `MsgAggregateDoRatePrevote`, containing the SHA256 hash of the exchange rates of Do with respect to a do peg. A prevote must be submitted for all different denomination on which to report a Do exchange rates.
    * A `MsgAggregateDoRateVote`, containing the salt used to create the hash for the aggreagte prevote submitted in the previous interval `P_t-1`.
...
* Vote Tally
    ...
    For each denomination, if the total voting power of submitted votes exceeds 50%, the weighted median of the votes is recorded on-chain as the effective exchange rate for Do against that denomination for the following `VotePeriod` `P_t+1`.
...
* The validator fails to submits a vote for Do exchange rate against **each and every** denomination specified in `Whitelist`.
...
> The control flow for vote-tallying, Do exchange rate updates, ballot rewards and slashing happens at the end of every `VotePeriod`, and is found at the [end-block ABCI](./03_end_block.md) function rather than inside message handlers.

...
func InitGenesis(ctx sdk.Context, keeper keeper.Keeper, data *types.GenesisState) {
	...
	for _, ex := range data.ExchangeRates {
		keeper.SetDoExchangeRate(ctx, ex.Denom, ex.ExchangeRate)
	}
	...
	for _, ap := range data.AggregateExchangeRatePrevotes {
		...
		keeper.SetAggregateDoRatePrevote(ctx, valAddr, ap)
	}
	...
	for _, av := range data.AggregateExchangeRateVotes {
		...
		keeper.SetAggregateDoRateVote(ctx, valAddr, av)
	}
...
func ExportGenesis(ctx sdk.Context, keeper keeper.Keeper) *types.GenesisState {
	...
	exchangeRates := []types.DoRateTuple{}
	keeper.IterateDoExchangeRates(ctx, func(denom string, rate math.LegacyDec) (stop bool) {
		exchangeRates = append(exchangeRates, types.DoRateTuple{Denom: denom, ExchangeRate: rate})
		return false
	})
	...
	aggregateExchangeRatePrevotes := []types.AggregateDoRatePrevote{}
	keeper.IterateAggregateDoRatePrevotes(ctx, func(_ sdk.ValAddress, aggregatePrevote types.AggregateDoRatePrevote) (stop bool) {
		aggregateExchangeRatePrevotes = append(aggregateExchangeRatePrevotes, aggregatePrevote)
		return false
	})

	aggregateExchangeRateVotes := []types.AggregateDoRateVote{}
	keeper.IterateAggregateDoRateVotes(ctx, func(_ sdk.ValAddress, aggregateVote types.AggregateDoRateVote) bool {

...
func NewDecodeStore(cdc codec.Codec) func(kvA, kvB kv.Pair) string {
	return func(kvA, kvB kv.Pair) string {
		...
		case bytes.Equal(kvA.Key[:1], types.AggregateDoRatePrevoteKey):
			var prevoteA, prevoteB types.AggregateDoRatePrevote
			cdc.MustUnmarshal(kvA.Value, &prevoteA)
			cdc.MustUnmarshal(kvB.Value, &prevoteB)
			return fmt.Sprintf("%v\n%v", prevoteA, prevoteB)
		case bytes.Equal(kvA.Key[:1], types.AggregateDoRateVoteKey):
			var voteA, voteB types.AggregateDoRateVote

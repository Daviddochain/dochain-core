...
6. Finally, record the Do issuance with `k.RecordEpochInitialIssuance()`. This will be used in calculating the seigniorage for the next epoch.
...
This function gets run at the end of an epoch  and records the current values of tax rewards $T$, seigniorage rewards $S$, and total staked Do $\Sigma$ as the historic indicators for epoch $t$ before moving to the next epoch $t+1$.
...
1. Calculate the rolling average $\tau _y$ of Tax Rewards per unit Do over the last year `WindowLong`.

2. Calculate the rolling average $\tau _m$` of Tax Rewards per unit Do over the last month `WindowShort`.
...
1. The seigniorage $\Sigma$ of the current epoch is calculated by taking the difference between the Do supply at the start of the epoch ([Epoch Initial Issuance](./02_state.md#EpochInitialIssuance)) and the Do supply at the time of calling.

   Note that $\Sigma > 0$ when the current Do supply is lower than at the start of the epoch, because the Do had been burned from Do swaps into do. See [here](../../market/spec/01_concepts.md#Seigniorage).

2. The Reward Weight $w$ is the percentage of the seigniorage designated for ballot rewards. Amount $S$ of new Do is minted, and the [`Oracle`](../../oracle/spec/README.md) module receives $S = \Sigma * w$ of the seigniorage.

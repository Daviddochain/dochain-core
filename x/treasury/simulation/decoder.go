...
func NewDecodeStore(cdc codec.Codec) func(kvA, kvB kv.Pair) string {
	return func(kvA, kvB kv.Pair) string {
		...
		case bytes.Equal(kvA.Key[:1], types.TSLKey):
			var TotalStakedDoA, TotalStakedDoB sdk.IntProto
			cdc.MustUnmarshal(kvA.Value, &TotalStakedDoA)
			cdc.MustUnmarshal(kvB.Value, &TotalStakedDoB)
			return fmt.Sprintf("%v\n%v", TotalStakedDoA, TotalStakedDoB)
		default:

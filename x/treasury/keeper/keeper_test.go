...
func TestMicroLunaIssuance(t *testing.T) {
	...
	initialSupply := input.BankKeeper.GetSupply(input.Ctx, core.MicroDoDenom)
	// See that we can get and set do issuance
	...
	for i := int64(0); i < 10; i++ {
		...
		require.Equal(t, initialSupply.Amount.Add(sdkmath.NewInt(i)), input.TreasuryKeeper.GetEpochInitialIssuance(input.Ctx).AmountOf(core.MicroDoDenom))

		input.BankKeeper.MintCoins(input.Ctx, faucetAccountName, sdk.NewCoins(sdk.NewCoin(core.MicroDoDenom, sdkmath.OneInt())))
	}
}
...
func TestPeekEpochSeigniorage(t *testing.T) {
	...
	for i := int64(0); i < 10; i++ {
		...
		err := input.BankKeeper.MintCoins(input.Ctx, faucetAccountName, sdk.NewCoins(sdk.NewCoin(core.MicroDoDenom, issueAmount)))
		require.NoError(t, err)

		burnAmount := sdkmath.NewInt(rand.Int63()%(faucetBalance.Amount.Int64()+issueAmount.Int64()) + 1)
		err = input.BankKeeper.BurnCoins(input.Ctx, faucetAccountName, sdk.NewCoins(sdk.NewCoin(core.MicroDoDenom, burnAmount)))
		require.NoError(t, err)

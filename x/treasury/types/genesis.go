...
// NewGenesisState creates a new treasury GenesisState object.
...
// DefaultGenesisState returns the default treasury genesis state.
...
func ValidateGenesis(data *GenesisState) error {
	if data.TaxRate.LT(data.Params.TaxPolicy.RateMin) || data.TaxRate.GT(data.Params.TaxPolicy.RateMax) {
		return fmt.Errorf(
			"tax_rate must be less than RateMax(%s) and greater than RateMin(%s)",
			data.Params.TaxPolicy.RateMax,
			data.Params.TaxPolicy.RateMin,
		)
	}

	if data.RewardWeight.LT(data.Params.RewardPolicy.RateMin) || data.RewardWeight.GT(data.Params.RewardPolicy.RateMax) {
		return fmt.Errorf(
			"reward_weight must be less than RateMax(%s) and greater than RateMin(%s)",
			data.Params.RewardPolicy.RateMax,
			data.Params.RewardPolicy.RateMin,
		)
	}
...
// GetGenesisStateFromAppState returns the treasury GenesisState from the raw

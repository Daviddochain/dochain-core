...
func TestEndBlockerIssuanceUpdateWithBurnModule(t *testing.T) {
	...
	supply := input.BankKeeper.GetSupply(input.Ctx, core.MicroDoDenom)
	...
	require.Equal(t,
		...
		supply.Amount.Sub(keeper.InitCoins.AmountOf(core.MicroDoDenom)),
		issuance.AmountOf(core.MicroDoDenom))
}

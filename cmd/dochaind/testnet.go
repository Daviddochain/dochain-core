...
func testnetCmd(mbm module.BasicManager, genBalIterator banktypes.GenesisBalancesIterator) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a dochaind testnet",
...
Example:
	dochaind testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "dochaind", "Home directory of the node's daemon configuration")
	cmd.Flags().String(flagStartingIPAddress, "192.168.0.1", "Starting IP address (192.168.0.1 results in persistent peers list ID0@192.168.0.1:46656, ID1@192.168.0.2:46656, ...)")
	cmd.Flags().String(flags.FlagChainID, "", "genesis file chain-id, if left blank will be randomly created")
	cmd.Flags().String(server.FlagMinGasPrices, fmt.Sprintf("0.000006%s", appparams.BondDenom), "Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 0.01photino,0.001udo)")
...
func InitTestnet(
	...
	doappConfig := appConfig.(DoAppConfig)
	doappConfig.MinGasPrices = minGasPrices
	doappConfig.API.Enable = true
	doappConfig.Telemetry.Enabled = true
	doappConfig.Telemetry.PrometheusRetentionTime = 60
	doappConfig.Telemetry.EnableHostnameLabel = false
	doappConfig.Telemetry.GlobalLabels = [][]string{{"chain_id", chainID}}
	...
	for i := 0; i < numValidators; i++ {
		...
		srvconfig.WriteConfigFile(filepath.Join(nodeDir, "config/app.toml"), doappConfig)
	}

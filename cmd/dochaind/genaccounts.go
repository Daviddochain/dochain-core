...
func AddGenesisAccountCmd(defaultNodeHome string) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: fmt.Sprintf(`Add a genesis account to genesis.json. The provided account must specify
...
Example:
$ dochaind add-genesis-account acc1 '10000000000%s' \
  --vesting-amount '10000000000%s' \
  --vesting-schedules '%s|30|60|1'

Or add a normal account:
...
`, core.MicroDoDenom, core.MicroDoDenom, core.MicroDoDenom, core.MicroDoDenom),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				...
				// Attempt to look up address from keyring if no address was provided.
			...
			if !vestingAmt.IsZero() {
				...
				for _, unparsedSchedule := range unparsedSchedules {
					...
					if vs, ok := vestingSchedulesDenomMap[denom]; ok {
						...
						vestingSchedulesDenomMap[denom] = &vestingtypes.VestingSchedule{
							Denom:     denom,
							Schedules: vestingtypes.Schedules{lazySchedule},
						}
					}
				}
	...
	cmd.Flags().String(flagVestingSchedules, "", "comma separated vesting schedules [denom|start|end|ratio][,[denom|start|end|ratio]], where 'start' and 'end' use day units")

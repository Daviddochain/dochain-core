...
# Configuration
FACTORY_CONTRACT_PATH="scripts/wasm/dex/artifacts/dochainswap_factory.wasm"
PAIR_CONTRACT_PATH="scripts/wasm/dex/artifacts/dochainswap_pair.wasm"
ROUTER_CONTRACT_PATH="scripts/wasm/dex/artifacts/dochainswap_router.wasm"
TOKEN_CONTRACT_PATH="scripts/wasm/dex/artifacts/dochainswap_token.wasm"
...
FACTORY_CONTRACT_ADDRESS=$(instantiate_contract "$FACTORY_CONTRACT_PATH" "$FACTORY_CODE_ID" "$FACTORY_MSG" "LUNC Do Chainswap Factory")
echo "Factory contract address: $FACTORY_CONTRACT_ADDRESS"

# Create router instantiation message
ROUTER_MSG=$(cat << EOF
{
  "dochainswap_factory": "$FACTORY_CONTRACT_ADDRESS"
}
EOF
)

# Instantiate router contract
ROUTER_CONTRACT_ADDRESS=$(instantiate_contract "$ROUTER_CONTRACT_PATH" "$ROUTER_CODE_ID" "$ROUTER_MSG" "LUNC Do Chainswap Router")
...
msg="{\"add_native_token_decimals\":{\"denom\":\"udotest\",\"decimals\":6}}"

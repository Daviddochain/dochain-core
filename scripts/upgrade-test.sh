...
# $(curl --silent "https://api.github.com/repos/Daviddochain/dochain-core/releases/latest" | jq -r '.tag_name')
...
if [[ "$FORK" == "true" ]]; then
    if [ -n "$FORK_HALT_HEIGHT" ]; then
        export DOCHAIND_HALT_HEIGHT="$FORK_HALT_HEIGHT"
    elif [ -n "$ADDITIONAL_PRE_SCRIPTS" ]; then
        # Allow pre-upgrade scripts (e.g. wasm deploy) enough blocks before halting.
        export DOCHAIND_HALT_HEIGHT=200
    else
        export DOCHAIND_HALT_HEIGHT=20
    fi
    echo "FORK mode: DOCHAIND_HALT_HEIGHT=$DOCHAIND_HALT_HEIGHT"
fi

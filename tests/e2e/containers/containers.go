...
func (m *Manager) RunHermesResource(chainAID, dochainARelayerNodeName, dochainAValMnemonic, chainBID, dochainBRelayerNodeName, dochainBValMnemonic string, hermesContainerName string, hermesCfgPath string) (*dockertest.Resource, error) {
	hermesResource, err := m.pool.RunWithOptions(
		&dockertest.RunOptions{
			...
			Env: []string{
				fmt.Sprintf("DOCHAIN_A_E2E_CHAIN_ID=%s", chainAID),
				fmt.Sprintf("DOCHAIN_B_E2E_CHAIN_ID=%s", chainBID),
				fmt.Sprintf("DOCHAIN_A_E2E_VAL_MNEMONIC=%s", dochainAValMnemonic),
				fmt.Sprintf("DOCHAIN_B_E2E_VAL_MNEMONIC=%s", dochainBValMnemonic),
				fmt.Sprintf("DOCHAIN_A_E2E_VAL_HOST=%s", dochainARelayerNodeName),
				fmt.Sprintf("DOCHAIN_B_E2E_VAL_HOST=%s", dochainBRelayerNodeName),
			},
			Entrypoint: []string{
				"sh",
				"-c",

...
depending on the do codebase. It initializes the chains for testing
via Docker files. As a result, the test suite may provide the desired
do version to Docker containers during the initialization.
...
such as the `app.toml`. This package directly depends on the do
codebase.
...
production do container to start the `dochaind` service.
...
- `DO_E2E_SKIP_IBC` - when true, skips the IBC tests tests.

- `DO_E2E_SKIP_STATE_SYNC` - when true, skips the state sync tests.

- `DO_E2E_SKIP_CLEANUP` - when true, avoids cleaning up the e2e Docker
containers.

- `DO_E2E_FORK_HEIGHT` - when the above "IS_FORK" env variable is set to true, this is the string
of the height in which the network should fork. This should match the ForkHeight set in constants.go

- `DO_E2E_UPGRADE_VERSION` - string of what version will be upgraded to (for example, "v10")

- `DO_E2E_DEBUG_LOG` - when true, prints debug logs from executing CLI commands
...
```json
{
    ...
    "buildFlags": "-tags e2e",
    "env": {
        "DO_E2E_SKIP_IBC": "true",
        "DO_E2E_SKIP_CLEANUP": "true",
        "DO_E2E_SKIP_STATE_SYNC": "true",
        "DO_E2E_UPGRADE_VERSION": "v5",
        "DO_E2E_DEBUG_LOG": "true",
        "DO_E2E_FORK_HEIGHT": ""
    }
}
```

...
// TestDoGaiaOsmoPFM validates a multi-hop MsgTransfer from do -> Osmosis -> Gaia via Packet Forward Middleware.
// Mirrors TestDoPFM behavior and logging.
func TestDoGaiaOsmoPFM(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
	...
	ic := interchaintest.NewInterchain().
		...
		AddLink(interchaintest.InterchainLink{Chain1: do, Chain2: osmo, Relayer: r, Path: pathDoOsmo}).
	...
	require.NoError(t, r.StartRelayer(ctx, eRep, pathDoOsmo, pathOsmoGaia))
	...
	chDo ChainOsmo, err := ibc.GetTransferChannel(ctx, r, eRep, do.Config().ChainID, osmo.Config().ChainID)
	require.NoError(t, err)
	...
	osmoFirstHopPath := transfertypes.GetPrefixedDenom(chDo ChainOsmo.Counterparty.PortID, chDo ChainOsmo.Counterparty.ChannelID, do.Config().Denom)
	...
	dochainBalBefore, err := do.GetBalance(ctx, doUser.FormattedAddress(), do.Config().Denom)
	require.NoError(t, err)
	...
	t.Logf("initial balances: do(%s)=%s, gaia(%s)=%s, osmo(%s)=%s",
		do.Config().Denom, dochainBalBefore.String(),
	...
	t.Logf("do->osmo channel (do side)=%s, (osmo side)=%s", chDo ChainOsmo.ChannelID, chDo ChainOsmo.Counterparty.ChannelID)
	...
	transferTx, err := do.SendIBCTransfer(ctx, chDo ChainOsmo.ChannelID, doUser.KeyName(), transfer, ibc.TransferOptions{Memo: memo})
	require.NoError(t, err)

	doH, err := do.Height(ctx)
	require.NoError(t, err)
	_, err = testutil.PollForAck(ctx, do, doH-5, doH+200, transferTx.Packet)
	if err != nil {
	...
	dochainBalAfter, err := do.GetBalance(ctx, doUser.FormattedAddress(), do.Config().Denom)
	require.NoError(t, err)
	require.LessOrEqual(t, dochainBalAfter.Int64(), dochainBalBefore.Sub(amount).Int64())
	t.Logf("source balance after send: do(%s)=%s", do.Config().Denom, dochainBalAfter.String())
...
// TestDoPFM validates a multi-hop MsgTransfer from do -> Osmosis -> Do Chain2 via Packet Forward Middleware.
// Both do chains run Cosmos SDK v0.53, satisfying the requirement to validate between two v0.53 chains.
func TestDoPFM(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
	...
	// do (source)
	doCfg1, err := createConfig()
	require.NoError(t, err)

	// do (destination)
	doCfg2 := doCfg1.Clone()
	doCfg2.Name = "core-counterparty"
	doCfg2.ChainID = "core-counterparty-1"
	...
	osmo := chains[1].(*cosmos.CosmosChain)   // hop with PFM
	dochain2 := chains[2].(*cosmos.CosmosChain) // destination

	r := interchaintest.NewBuiltinRelayerFactory(ibc.CosmosRly, zaptest.NewLogger(t)).Build(t, client, network)

	const (
		pathDoOsmo  = "do-osmo"
		pathOsmoDo2 = "osmo-dochain2"
	)
	...
	ic := interchaintest.NewInterchain().
		...
		AddChain(dochain2).
		AddRelayer(r, "relayer").
		AddLink(interchaintest.InterchainLink{Chain1: do1, Chain2: osmo, Relayer: r, Path: pathDoOsmo}).
		AddLink(interchaintest.InterchainLink{Chain1: osmo, Chain2: dochain2, Relayer: r, Path: pathOsmoDo2})
	...
	require.NoError(t, r.StartRelayer(ctx, eRep, pathDoOsmo, pathOsmoDo2))
	t.Cleanup(func() { _ = r.StopRelayer(ctx, eRep) })

	// Users on each chain
	users := interchaintest.GetAndFundTestUsers(t, ctx, "default", math.NewInt(genesisWalletAmount), do1, osmo, dochain2)
	do1User := users[0]
	osmoUser := users[1]
	dochain2User := users[2]

	require.NoError(t, testutil.WaitForBlocks(ctx, 8, do1, osmo, dochain2))

	// Channels for both hops
	chdo1Osmo, err := ibc.GetTransferChannel(ctx, r, eRep, do1.Config().ChainID, osmo.Config().ChainID)
	require.NoError(t, err)
	chOsmoDo Chain2, err := ibc.GetTransferChannel(ctx, r, eRep, osmo.Config().ChainID, dochain2.Config().ChainID)
	require.NoError(t, err)

	// Compute final IBC denom on dochain2 after two hops (before sending)
	...
	secondHopFullPath := transfertypes.GetPrefixedDenom(chOsmoDo Chain2.PortID, chOsmoDo Chain2.ChannelID, osmoFirstHopPath)
	dt2 := transfertypes.ParseDenomTrace(secondHopFullPath)
	finalIBCDenom := dt2.IBCDenom()
	// For Osmosis, compute IBC hash denom for the first-hop voucher to check any transient balances
	dtOsmo := transfertypes.ParseDenomTrace(osmoFirstHopPath)
	...
	require.NoError(t, err)
	dochain2Before, err := dochain2.GetBalance(ctx, dochain2User.FormattedAddress(), finalIBCDenom)
	require.NoError(t, err)
	osmoBefore, err := osmo.GetBalance(ctx, osmoUser.FormattedAddress(), osmoIbcDenom)
	require.NoError(t, err)
	t.Logf("initial balances: do1(%s)=%s, dochain2(%s)=%s, osmo(%s)=%s",
		do1.Config().Denom, do1BalBefore.String(),
		finalIBCDenom, dochain2Before.String(),
		osmoIbcDenom, osmoBefore.String())

	// Build memo to forward from osmosis -> dochain2 (use Osmosis-side port/channel)
	memo := forwardMemo(dochain2User.FormattedAddress(), chOsmoDo Chain2.PortID, chOsmoDo Chain2.ChannelID, "600s")

	// Diagnostics: log memo and channel ids
	t.Logf("PFM memo (do1->osmo->dochain2): %s", memo)
	t.Logf("do1->osmo channel (do1 side)=%s, (osmo side)=%s", chdo1Osmo.ChannelID, chdo1Osmo.Counterparty.ChannelID)
	t.Logf("osmo->dochain2 channel (osmo side)=%s, (dochain2 side)=%s", chOsmoDo Chain2.ChannelID, chOsmoDo Chain2.Counterparty.ChannelID)
	...
	require.NoError(t, testutil.WaitForBlocks(ctx, 24, do1, osmo, dochain2))
	...
	// Destination (dochain2) balance should reflect forwarded amount, allow extra blocks in case of relayer delay
	if waitBalanceEq(t, ctx, dochain2, dochain2User.FormattedAddress(), finalIBCDenom, dochain2Before.Add(amount), 30, do1, osmo, dochain2) {
		dochain2After, err := dochain2.GetBalance(ctx, dochain2User.FormattedAddress(), finalIBCDenom)
		require.NoError(t, err)
		osmoAfter, err := osmo.GetBalance(ctx, osmoUser.FormattedAddress(), osmoIbcDenom)
		require.NoError(t, err)
		t.Logf("post-forward balances: dochain2(%s)=%s (expected=%s), osmo(%s)=%s",
			finalIBCDenom, dochain2After.String(), dochain2Before.Add(amount).String(),
			osmoIbcDenom, osmoAfter.String())
		return
	} else {
		// If Osmosis did not emit a send_packet on the osmo->dochain2 channel, likely PFM is not enabled; skip to avoid false failures.
		osmoEnd, _ := osmo.Height(ctx)
		osmoStart := osmoEnd - 200
		if osmoStart < 1 {
			osmoStart = 1
		}
		forwarded := hasSendPacket(t, ctx, osmo, osmoStart, osmoEnd, chOsmoDo Chain2.PortID, chOsmoDo Chain2.ChannelID)
		t.Logf("osmo->dochain2 send_packet observed on %s/%s in [%d,%d]: %v", chOsmoDo Chain2.PortID, chOsmoDo Chain2.ChannelID, osmoStart, osmoEnd, forwarded)
		if !forwarded {
			t.Skipf("skipping: osmosis did not forward (no send_packet on %s/%s); PFM likely not enabled in image", chOsmoDo Chain2.PortID, chOsmoDo Chain2.ChannelID)
		}
		if seq2, pkt2, ok2 := findSendPacketWithSeq(t, ctx, osmo, osmoStart, osmoEnd, chOsmoDo Chain2.PortID, chOsmoDo Chain2.ChannelID); ok2 {
			t.Logf("second hop packet_data (osmo->dochain2): seq=%d receiver=%s denom=%s amount=%s", seq2, pkt2.Receiver, pkt2.Denom, pkt2.Amount)
			require.Equal(t, dochain2User.FormattedAddress(), pkt2.Receiver)
			require.Equal(t, amount.String(), pkt2.Amount)
			// Denom in packet_data on Osmosis must use Osmosis-side channel (counterparty of do's channel)
			require.Equal(t, osmoFirstHopPath, pkt2.Denom)
			// Scan destination chain for recv_packet with matching sequence
			dochain2H, _ := dochain2.Height(ctx)
			dochain2Start := dochain2H - 200
			if dochain2Start < 1 {
				dochain2Start = 1
			}
			if hasRecvPacketOnDestBySeq(t, ctx, dochain2, dochain2Start, dochain2H, chOsmoDo Chain2.Counterparty.PortID, chOsmoDo Chain2.Counterparty.ChannelID, seq2) {
				t.Logf("destination recv_packet observed on dochain2 for seq=%d in [%d,%d] on %s/%s", seq2, dochain2Start, dochain2H, chOsmoDo Chain2.Counterparty.PortID, chOsmoDo Chain2.Counterparty.ChannelID)
			} else {
				t.Logf("destination recv_packet NOT observed on dochain2 for seq=%d in [%d,%d] on %s/%s", seq2, dochain2Start, dochain2H, chOsmoDo Chain2.Counterparty.PortID, chOsmoDo Chain2.Counterparty.ChannelID)
			}
			if ack, ok := findWriteAckOnDestBySeq(t, ctx, dochain2, dochain2Start, dochain2H, chOsmoDo Chain2.Counterparty.PortID, chOsmoDo Chain2.Counterparty.ChannelID, seq2); ok {
				if okSucc, parsed := parseAckSuccess(ack); parsed {
					t.Logf("destination write_acknowledgement on dochain2 for seq=%d indicates success=%v", seq2, okSucc)
				} else {
					t.Logf("destination write_acknowledgement on dochain2 (seq=%d) present but could not parse ack format: %q", seq2, ack)
				}
			} else {
				t.Logf("destination write_acknowledgement NOT observed on dochain2 for seq=%d in [%d,%d] on %s/%s", seq2, dochain2Start, dochain2H, chOsmoDo Chain2.Counterparty.PortID, chOsmoDo Chain2.Counterparty.ChannelID)
			}
			// Dump balances and denom trace for debugging when delivery seems stuck
			dumpBalances(t, ctx, dochain2, dochain2User.FormattedAddress())
			logDenomTrace(t, ctx, dochain2, finalIBCDenom)
		} else {
			t.Logf("could not locate packet_data+sequence for osmo->dochain2 send_packet in [%d,%d]", osmoStart, osmoEnd)
		}
		// Delivery did not complete within the initial window; treat as relayer nondelivery and skip
		t.Skipf("skipping: osmosis forwarded but delivery to dochain2 did not complete within window; likely relayer nondelivery")
	}
}

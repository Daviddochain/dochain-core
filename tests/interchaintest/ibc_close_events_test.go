...
func TestIBCv2ChannelCloseEvents(t *testing.T) {
	...
	require.NoError(t, err)
	dochainA := chains[0].(*cosmos.CosmosChain)
	dochainB := chains[1].(*cosmos.CosmosChain)

	r := interchaintest.NewBuiltinRelayerFactory(ibc.CosmosRly, zaptest.NewLogger(t)).Build(t, client, network)
	const path = "do-dochain2-close"
	ic := interchaintest.NewInterchain().
		AddChain(dochainA).
		AddChain(dochainB).
		AddRelayer(r, "relayer").
		AddLink(interchaintest.InterchainLink{Chain1: dochainA, Chain2: dochainB, Relayer: r, Path: path})
	...
	require.NoError(t, testutil.WaitForBlocks(ctx, 5, dochainA, dochainB))
	...
	require.NoError(t, testutil.WaitForBlocks(ctx, 6, dochainA, dochainB))

	// scan recent blocks for close events
	ah, _ := dochainA.Height(ctx)
	bh, _ := dochainB.Height(ctx)
	startA := ah - 30
	if startA < 1 {
		startA = 1
	}
	...
	require.True(t, containsAnyEventInWindow(t, ctx, dochainA, startA, ah, "channel_close_init"))
	require.True(t, containsAnyEventInWindow(t, ctx, dochainB, startB, bh, "channel_close_confirm"))
}

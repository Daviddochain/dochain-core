...
func TestValidator(t *testing.T) {
	...
	doValidators, pubKeys, err := helpers.UnmarshalValidators(*config.EncodingConfig, stdout)
	require.NoError(t, err)
	require.Equal(t, 5, len(doValidators.Validators))

	// find exactly one jailed validator and capture its consensus pubkey
	var val1PubKey cryptotypes.PubKey
	count := 0
	for i, val := range doValidators.Validators {
		if val.Jailed {
			count++

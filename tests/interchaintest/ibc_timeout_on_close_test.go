...
func TestIBCv2TimeoutOnClose(t *testing.T) {
	...
	dochainBefore, err := do.GetBalance(ctx, doUser.FormattedAddress(), do.Config().Denom)
	require.NoError(t, err)
	...
	_, err = do.Validators[0].ExecTx(ctx, doUser.KeyName(), "ibc-transfer", "transfer", "transfer", ch.ChannelID, gaiaUser.FormattedAddress(), amountStr, "--packet-timeout-timestamp", fmt.Sprintf("%d", timeoutNs), "--absolute-timeouts")
	require.NoError(t, err)
	require.NoError(t, testutil.WaitForBlocks(ctx, 3, do, gaia))

	dochainAfterSend, err := do.GetBalance(ctx, doUser.FormattedAddress(), do.Config().Denom)
	require.NoError(t, err)
	// Balance should have dropped by at least amount (plus gas)
	require.Less(t, dochainAfterSend.Int64(), dochainBefore.Sub(amount).Int64())
	...
	require.True(t, containsAnyEventInWindow(t, ctx, do, start, doH, "timeout_packet"))
	...
	dochainAfterTimeout, err := do.GetBalance(ctx, doUser.FormattedAddress(), do.Config().Denom)
	require.NoError(t, err)
	require.GreaterOrEqual(t, dochainAfterTimeout.Int64(), dochainAfterSend.Add(amount).Int64())
}
